...
public class Guia7EjeExtra06 {
    ...
    public static void main(String[] args) {
        ...
        ServicioAhorcado servAhor = new ServicioAhorcado();
        Ahorcado ahor = servAhor.crearJuego();
        
        System.out.println("display: " + ahor.toString());
        
        jugar(servAhor, ahor);
        
    }

    private static void jugar(ServicioAhorcado servAhor, Ahorcado ahor) {

        while((ahor.getCantidadJugadasMax() > 0 ) && (ahor.getCantidadLetrasEncontradas() != ahor.getCantidadLetras())){
            servAhor.buscar(ahor);
            servAhor.longitud(ahor);
            servAhor.intentos(ahor);
            
        }
        if(ahor.getCantidadLetrasEncontradas() != ahor.getCantidadLetras()){
